...
class Skribble:
    ...
    # HELPER METHODS
    #########################
    # validate response is image/png
    def valid_type(self, raw_asset, response):
        ...
        try:
            if response.headers['Content-Type'] == mime_type:
                ...
                logger.error(
                    'Invalid type for {}, expected {}, instead saw {}...'.format(raw_asset['media_id'], mime_type,
    ...
    def validate_checksum(self, raw_asset, response):
        ...
        if type_of_check == 'sha1':
            ...
            raise Exception('Unknown checksum type {}...'.format(type_of_check))
        ...
        if hash_value == value:
            ...
            logger.error(
                '{} failed checksum validation, expected checksum {}, instead saw {}...'.format(raw_asset['media_id'],
                                                                                                value, hash_value))
            raise Exception(
                '{} failed checksum validation, expected checksum {}, instead saw {}...'.format(raw_asset['media_id'],
                                                                                                value, hash_value))
        ...
        for asset in processed_assets:
            ...
            if processed_base_asset['raw'] != asset['raw']:
                if (not processed_base_asset['raw']['can_overlap']) | (not asset['raw']['can_overlap']):
                    if (base_corners['top_left'][0] >= asset_corners['top_left'][0]) | (
                        asset_corners['top_left'][0] >= base_corners['top_left'][0]):
                        if (base_corners['top_left'][0] <= asset_corners['top_right'][0]) | (
                            asset_corners['top_left'][0] <= base_corners['top_right'][0]):
                            if (base_corners['top_left'][1] >= asset_corners['top_left'][1]) | (
                                asset_corners['top_left'][1] >= base_corners['top_left'][1]):
                                if (base_corners['top_left'][1] <= asset_corners['bottom_left'][1]) | (
                                    asset_corners['top_left'][1] <= base_corners['bottom_left'][1]):
                                    logger.error('Error, collision detected between {} and {}...'.format(
                                        processed_base_asset['raw']['media_id'], asset['raw']['media_id']))
                                    raise Exception('Error, collision detected between {} and {}...'.format(
                                        processed_base_asset['raw']['media_id'], asset['raw']['media_id']))
            logger.debug('No collision detected between {} and {}...'.format(processed_base_asset['raw']['media_id'],
                                                                             asset['raw']['media_id']))
    ...
    def recalculate_coordinates(self, processed_asset):
        ...
        try:
            ...
            nx = processed_asset['left']
            ny = processed_asset['top']
            # if asset has shrunk
    ...
    def report_to_api(self, status):
        ...
        try:
            ...
            if response.status_code != 201:
                ...
                raise Exception(
                    'Unexpected response code from API, expected 201, saw {}...'.format(response.status_code))
    ...
    def preflight_background(self, base, raw_asset):
        ...
        try:
            ...
            logger.error('Background {} failed preflight, verify source URL and image type.'.format(
                raw_asset['raw']['media_id']))
            raise
